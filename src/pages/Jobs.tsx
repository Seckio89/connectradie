...
export default function Jobs({ embedded = false }: { embedded?: boolean }) {
  ...
  // Tone, not colour classes — the Pill primitive owns the styling. Teal wherever
  // money is moving as agreed, rose on declined/cancelled, amber while the job is
  // still waiting on someone. `cancelled` was rose alongside `declined`; it stays
  // rose because for a tradie a cancelled job IS a loss, not a neutral end state.
  const getStatusTone = (status: string | null): PillTone => {
    switch (status) {
      case 'completed':
      case 'funded':
      case 'in_progress':
  ...
  const content = (
    <>
      <div>
        {paymentSuccess && (
          <div className="mb-4 flex items-center gap-3 px-4 py-3 bg-ct-teal/[0.14] border border-ct-teal/30 rounded-ct-md">
            <CheckCircle2 className="w-5 h-5 text-ct-teal flex-shrink-0" />
            <p className="text-sm font-medium text-ct-teal">{paymentSuccess}</p>
          </div>
        )}
        {acceptSuccess && (
          <div className="mb-4 bg-ct-teal/[0.14] border border-ct-teal/30 rounded-ct-md p-4">
            <div className="flex items-center gap-3 mb-2">
              <CheckCircle2 className="w-5 h-5 text-ct-teal flex-shrink-0" />
              <p className="text-sm font-bold text-ct-teal">Job accepted!</p>
            </div>
            <p className="text-xs text-ct-teal ml-8">Next: Contact the client via Messages to confirm the details. Once payment is secured, you can mark the job complete when finished.</p>
          </div>
        )}
        {offlineQueued && (
        ...
        {operationError && (
          <div className="mb-4 flex items-center gap-3 px-4 py-3 bg-ct-rose/[0.13] border border-ct-rose/[0.34] rounded-ct-md">
            <AlertCircle className="w-5 h-5 text-ct-rose flex-shrink-0" />
            <p className="text-sm font-medium text-ct-paper">{operationError}</p>
            <button onClick={() => setOperationError(null)} className="ml-auto text-ct-rose hover:text-ct-rose">
        ...
        {!embedded && (
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-ct-paper">{isTradie ? 'My Jobs' : 'Active Jobs'}</h1>
        ...
        {isTradie && !profile?.stripe_connect_onboarding_complete && (
          <div className="mb-6 bg-ct-amber/[0.13] border border-ct-amber/[0.34] rounded-ct-lg p-4">
            <div className="flex items-center gap-3">
              <AlertCircle className="w-5 h-5 text-ct-amber flex-shrink-0" />
              <div className="flex-1">
                <p className="text-sm font-semibold text-ct-paper">Payouts not set up</p>
                <p className="text-xs text-ct-amber mt-0.5">You won&apos;t receive payments for completed jobs until your bank account is connected.</p>
              </div>
              <button
                onClick={() => navigate('/payouts')}
                className="flex-shrink-0 px-4 py-2 bg-ct-amber text-ct-ink text-sm font-semibold rounded-ct-md hover:bg-ct-amber transition-colors"
              >
                Set Up Payouts
              </button>
            </div>
                    ...
                    return (
                      <div key={monthKey}>
                        <button
                          ...
                          className="w-full flex items-center justify-between px-4 py-3 bg-ct-surface-2 rounded-ct-sm hover:bg-ct-surface-2 transition-colors mb-2"
                        >
                          <div className="flex items-center gap-2">
                            <h3 className="text-sm font-semibold text-ct-paper">{monthLabel}</h3>
                            <span className="px-3 py-1 rounded-full text-xs font-medium bg-ct-line text-ct-mute-2">
                              {monthJobs.length}
                            </span>
                          </div>
                            ...
                            {monthJobs.map((job) => {
                              return (
                                <div
                                  ...
                                  className="w-full text-left rounded-ct-md transition-all cursor-pointer border border-ct-line bg-ct-surface hover:border-ct-line hover:shadow-sm"
                                >
                                  {!isTradie ? (
                                  /* ── Client completed card: clean summary ── */
                                  <div className="px-5 py-4">
                                    <div className="flex items-start justify-between gap-3 mb-2">
                                      <h3 className="text-sm font-semibold text-ct-paper truncate flex-1 min-w-0">
                                        {job.title || job.description.match(/^\[([^\]]+)\]/)?.[1]?.replace(/_/g, ' ') || 'Job'}
                                      </h3>
                                      <Pill tone={paidJobIds.has(job.id) ? 'teal' : getStatusTone(job.status)} className="flex-shrink-0">
                                        {paidJobIds.has(job.id) ? 'Paid' : (job.status ?? 'pending').replace(/_/g, ' ')}
                                      </Pill>
                                    </div>
                                    <div className="space-y-1.5 text-sm text-ct-mute-2">
                                    ...
                                    {job.tradie_id && !reviewedJobIds.includes(job.id) && (
                                      <div className="mt-3 pt-3 border-t border-ct-line-soft" onClick={(e) => e.stopPropagation()}>
                                        <button
                                          onClick={(e) => { e.stopPropagation(); navigate(`/review/${job.id}`); }}
                                          className="inline-flex items-center gap-1.5 px-4 py-2 bg-ct-teal text-ct-ink rounded-ct-sm hover:brightness-110 text-sm font-medium transition-colors"
                                        >
                                  ...
                                  <div className="px-5 pt-4 pb-3">
                                    <div className="flex items-start justify-between gap-3 mb-2">
                                      <div className="flex-1 min-w-0">
                                        <h3 className="text-sm font-semibold text-ct-paper truncate">
                                          {job.title || job.description.match(/^\[([^\]]+)\]/)?.[1]?.replace(/_/g, ' ') || 'Job'}
                                        </h3>
                                      ...
                                      <Pill tone={getStatusTone(job.status)} className="flex-shrink-0">
                                        {(job.status ?? 'pending').replace(/_/g, ' ')}
                                      </Pill>
                                    </div>
                                    <p className="text-sm text-ct-mute-2 line-clamp-2 mb-3">
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-ct-mute">
                                      ...
                                      {job.budget_amount ? (
                                        <span className="font-ct-mono font-medium text-ct-paper">${job.budget_amount.toLocaleString()}</span>
                                  ...
                                  {job.status === 'completed' && !job.completion_notes && (
                                    <div className="px-5 pb-4 pt-1 border-t border-ct-line-soft" onClick={(e) => e.stopPropagation()}>
                                      <button
                                  ...
                                  {job.status === 'completed' && job.completion_notes && (
                                    <div className="px-5 pb-3 pt-1 border-t border-ct-line-soft" onClick={(e) => e.stopPropagation()}>
                                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${paidJobIds.has(job.id) ? 'text-ct-teal' : 'text-ct-mute'}`}>
                                        <CheckCircle2 className="w-3.5 h-3.5" />
            ...
            <div className="space-y-4">
              {jobs.map((job) => {
                ...
                <div
                  ...
                  className={`w-full text-left rounded-ct-md transition-all cursor-pointer ${
                    isFlashActive && isTradie
                      ? 'border-2 border-ct-teal shadow-[0_0_12px_rgba(251,191,36,0.2)] hover:shadow-[0_0_18px_rgba(251,191,36,0.3)] bg-ct-surface'
                      : 'border border-ct-line bg-ct-surface hover:border-ct-line hover:shadow-sm'
                  }`}
                >
                  {/* ── Header ── */}
                  <div className="px-5 pt-4 pb-3">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      {(() => {
                        // "Set price" is the one state on this row that is
                        // genuinely blocked on the tradie, so it stays amber
                        // even though the job itself is funded.
                        const needsPrice = job.status === 'funded'
                          && myQuotes.get(job.id)?.requires_site_inspection
                          && myQuotes.get(job.id)?.final_price == null;
                        return (
                          <Pill tone={needsPrice ? 'amber' : getStatusTone(job.status)} className="flex-shrink-0">
                            {needsPrice
                              ? 'Set price'
                  ...
                  {!isTradie && isFlashActive && job.status === 'pending' && (
                    <div className="mx-5 mb-3 flex items-center gap-2 px-3 py-2 bg-ct-amber/[0.13] border border-ct-amber/[0.34] rounded-ct-sm">
                      <Zap className="w-3.5 h-3.5 text-ct-teal flex-shrink-0" />
                      <span className="text-xs text-ct-amber">Boosting your job to find a Tradie faster</span>
                    </div>
                  )}

                  {job.status === 'declined' && job.decline_reason && (
                    <div className="mx-5 mb-3 px-3 py-2 bg-ct-rose/[0.13] border border-ct-rose/[0.34] rounded-ct-sm">
                      <p className="text-xs text-ct-rose"><span className="font-medium">Declined:</span> {job.decline_reason}</p>
                    </div>
                  )}
                  ...
                  {isTradie && job.status === 'pending' && job.client_contact_id && !job.client_id && (
                    <div className="px-5 pb-4 pt-1 border-t border-ct-line-soft flex items-center justify-between gap-2" onClick={(e) => e.stopPropagation()}>
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-ct-surface-2 text-ct-mute-2 rounded-ct-sm text-xs font-medium">
                        <Clock className="w-3.5 h-3.5" />
                        Waiting for your client to accept the quote
                      </span>
                      <button
                        onClick={(e) => { e.stopPropagation(); setWithdrawJob(job); }}
                        className="text-xs font-medium text-ct-rose hover:text-ct-rose whitespace-nowrap"
                      >
                        Withdraw
                      </button>
                    </div>
                  ...
                  {isTradie && job.status === 'pending' && !(job.client_contact_id && !job.client_id) && (
                    <div className="px-5 pb-4 pt-1 flex items-center gap-2 border-t border-ct-line-soft" onClick={(e) => e.stopPropagation()}>
                      {job.budget_type === 'to_be_quoted' || job.budget_type === 'request_quote' ? (
                        myQuotes.has(job.id) ? (
                          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-ct-surface-2 border border-ct-line rounded-ct-sm">
                            <CheckCircle2 className="w-3.5 h-3.5 text-ct-mute-2" />
                            <span className="text-xs font-medium text-ct-mute-2">
                            ...
                            <span className="text-xs text-ct-mute">· Awaiting response</span>
                          </div>
                        ) : (
                        <button
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-4 py-2 bg-ct-teal text-ct-ink rounded-ct-sm hover:brightness-110 disabled:opacity-50 text-sm font-medium transition-colors"
                        >
                      ...
                      <button
                        ...
                        disabled={actionLoading === job.id}
                        className="inline-flex items-center gap-1.5 px-3 py-2 text-ct-rose hover:bg-ct-rose/[0.13] rounded-ct-sm text-sm font-medium transition-colors"
                      >
                  ...
                  {isTradie && job.status === 'accepted' && (
                    <div className="px-5 pb-4 pt-1 flex items-center gap-2 flex-wrap border-t border-ct-line-soft" onClick={(e) => e.stopPropagation()}>
                      {job.client_contact_id && !job.client_id ? (
                        ...
                        <>
                          ...
                          {payLinkSent.has(job.id) && (
                            <span className="text-xs text-ct-mute">Link sent — you'll be notified when they pay.</span>
                          )}
                        </>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-ct-amber/[0.13] text-ct-amber rounded-ct-sm text-xs font-medium">
                          <Clock className="w-3.5 h-3.5" />
                  ...
                  {isTradie && (job.status === 'funded' || job.status === 'in_progress') && (
                    <div className="px-5 pb-4 pt-1 border-t border-ct-line-soft flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                      ...
                      {job.status === 'funded' && myQuotes.get(job.id)?.requires_site_inspection && myQuotes.get(job.id)?.final_price == null ? (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 bg-ct-amber/[0.13] text-ct-amber rounded-ct-sm hover:bg-ct-amber hover:text-ct-ink text-sm font-medium transition-colors"
                          >
                            <AlertCircle className="w-3.5 h-3.5" />
                            Set Final Price
                          </button>
                          <span className="text-xs text-ct-amber">After site visit</span>
                        </div>
                      ) : (
                      ...
                      {teamMembers.length > 0 && job.status === 'funded' && (
                        <>
                          {assignJobId === job.id ? (
                            <div className="w-full bg-ct-surface-2 rounded-ct-sm p-3 space-y-2">
                              <p className="text-xs font-medium text-ct-mute-2">Assign to team member:</p>
                              {teamMembers.map((m) => (
                                <button
                                  key={m.id}
                                  onClick={(e) => { e.stopPropagation(); handleAssignToTeam(job.id, m.id); }}
                                  disabled={assignLoading}
                                  className="w-full text-left flex items-center gap-2 px-3 py-2 rounded-ct-sm hover:bg-ct-surface text-sm transition-colors disabled:opacity-50"
                                >
                                  <User className="w-4 h-4 text-ct-mute" />
                                  <span className="font-medium text-ct-paper">{m.invite_name}</span>
                                  <span className="text-xs text-ct-mute capitalize">{m.role}</span>
                                </button>
                              ))}
                              <button
                                onClick={(e) => { e.stopPropagation(); setAssignJobId(null); }}
                                className="text-xs text-ct-mute hover:text-ct-mute-2"
                              >Cancel</button>
                            </div>
                          ) : (
                            <button
                              onClick={(e) => { e.stopPropagation(); setAssignJobId(job.id); }}
                              className="inline-flex items-center justify-center gap-2 px-4 py-2 border border-ct-line text-ct-mute-2 rounded-ct-sm hover:bg-ct-surface-2 transition-colors text-sm font-medium"
                            >
      ...
      {toast.show && (
        <div className={`fixed bottom-20 sm:bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:max-w-sm ${toast.isError ? 'bg-ct-rose' : 'bg-ct-teal'} text-ct-ink px-6 py-3 rounded-ct-md shadow-lg flex items-center gap-3 z-50 animate-slide-up`}>
          <div className={`w-2 h-2 ${toast.isError ? 'bg-ct-rose' : 'bg-ct-teal'} rounded-full animate-pulse`} />
