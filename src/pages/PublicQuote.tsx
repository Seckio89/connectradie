...
export default function PublicQuote() {
  ...
  return (
    <div className="min-h-screen bg-ct-surface-2 flex flex-col">
      <header className="bg-ct-surface border-b border-ct-line-soft">
      ...
      <main className="flex-1 max-w-2xl w-full mx-auto px-4 sm:px-6 py-8 sm:py-12">
        {loading ? (
          <div className="flex items-center justify-center py-20 text-ct-mute">
          ...
          <div className="bg-ct-surface rounded-ct-lg border border-ct-line p-8 text-center">
            <AlertCircle className="w-10 h-10 text-ct-mute mx-auto mb-3" />
            <h1 className="text-lg font-semibold text-ct-paper">Quote unavailable</h1>
            <p className="text-sm text-ct-mute mt-1">{error}</p>
          </div>
        ) : data ? (
          <div className="space-y-5">
            ...
            <div className="bg-ct-surface rounded-ct-lg border border-ct-line shadow-sm overflow-hidden">
              {/* Business identity */}
              <div className="p-5 sm:p-6 flex items-start gap-4 border-b border-ct-line-soft">
                <div className="w-14 h-14 rounded-ct-lg overflow-hidden bg-gradient-to-br from-ct-teal to-ct-teal flex items-center justify-center flex-shrink-0">
                  {showAvatar ? (
                    <img src={avatarUrl!} alt="" className="w-full h-full object-cover" onError={() => setAvatarFailed(true)} />
                  ) : (
                    <span className="text-xl font-bold text-ct-teal">{businessName.charAt(0).toUpperCase()}</span>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  {data.tradie.phone && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 mt-1.5 text-sm font-medium text-ct-teal hover:text-ct-teal"
                    >
              ...
              {/* Reference + dates */}
              <div className="px-5 sm:px-6 py-3 bg-ct-surface-2 border-b border-ct-line-soft flex flex-wrap items-center gap-x-6 gap-y-1 text-xs">
                {data.reference && <span className="text-ct-mute">Quote <span className="font-semibold text-ct-paper">{data.reference}</span></span>}
                {fmtDate(data.issuedDate) && <span className="text-ct-mute">Issued <span className="font-medium text-ct-mute-2">{fmtDate(data.issuedDate)}</span></span>}
                {fmtDate(data.validUntil) && <span className="text-ct-mute">Valid until <span className="font-medium text-ct-mute-2">{fmtDate(data.validUntil)}</span></span>}
              </div>
              ...
              {/* Prepared for */}
              {data.client?.name && (
                <div className="px-5 sm:px-6 py-3 border-b border-ct-line-soft text-xs">
                  <span className="text-ct-mute">Prepared for </span>
                  <span className="font-semibold text-ct-paper">{data.client.name}</span>
                  {data.client.location && <span className="text-ct-mute"> · {data.client.location}</span>}
                </div>
              )}
              ...
              {/* Price + GST */}
              <div className="px-5 sm:px-6 py-5 bg-ct-surface-2 border-b border-ct-line-soft">
                <div className="flex items-end justify-between gap-4">
                  <span className="text-sm font-medium text-ct-mute uppercase tracking-wide">Quoted price</span>
                  <span className="text-3xl font-bold text-ct-paper">{formatPrice(data.quote)}</span>
                </div>
                {data.quote.gstRegistered && definitePrice(data.quote) != null && (
                  <p className="text-right text-xs text-ct-mute mt-1">
              ...
              {(data.quote.proposedStartDate || data.quote.estimatedDuration || data.quote.includesMaterials != null || data.quote.callOutFee || data.quote.requiresSiteInspection) && (
                <div className="px-5 sm:px-6 py-4 border-b border-ct-line-soft grid grid-cols-2 gap-4">
              ...
              {data.quote.message && (
                <div className="p-5 sm:p-6">
                  <p className="text-xs font-medium text-ct-mute uppercase tracking-wide mb-1.5">Note from {businessName}</p>
                  <p className="text-sm text-ct-mute-2 whitespace-pre-line">{data.quote.message}</p>
                </div>
              )}
            </div>

            {actionError && (
              <div className="flex items-start gap-2.5 bg-ct-rose/[0.13] border border-ct-rose/[0.34] rounded-ct-lg p-4">
                <AlertCircle className="w-5 h-5 text-ct-rose flex-shrink-0 mt-0.5" />
                <p className="text-sm text-ct-rose">{actionError}</p>
              </div>
            )}
            ...
            {accepted ? (
              <div className="space-y-2.5">
                {released ? (
                  <div className="bg-ct-teal/[0.14] border border-ct-teal/30 rounded-ct-lg p-6 text-center">
                    <CheckCircle2 className="w-10 h-10 text-ct-teal mx-auto mb-2" />
                    <h3 className="font-semibold text-ct-teal">Payment released</h3>
                    <p className="text-sm text-ct-teal mt-1">
                  ...
                  <div className="bg-ct-teal/[0.14] border border-ct-teal/30 rounded-ct-lg p-6 text-center">
                    <CheckCircle2 className="w-10 h-10 text-ct-teal mx-auto mb-2" />
                    <h3 className="font-semibold text-ct-teal">Payment approved</h3>
                    <p className="text-sm text-ct-teal mt-1">
                  ...
                  <div className="bg-ct-surface border border-ct-line rounded-ct-lg p-6">
                    <div className="text-center">
                      <CheckCircle2 className="w-10 h-10 text-ct-teal mx-auto mb-2" />
                      <h3 className="font-semibold text-ct-paper">{businessName} marked this job complete</h3>
                    ...
                    <button
                      ...
                      disabled={releasing}
                      className="w-full mt-4 py-3.5 bg-ct-teal hover:brightness-110 text-ct-ink font-semibold rounded-ct-md disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                    >
                  ...
                  <div className="bg-ct-teal/[0.14] border border-ct-teal/30 rounded-ct-lg p-6 text-center">
                    <CheckCircle2 className="w-10 h-10 text-ct-teal mx-auto mb-2" />
                    <h3 className="font-semibold text-ct-teal">Quote accepted</h3>
                    <p className="text-sm text-ct-teal mt-1">
              ...
              <div className="bg-ct-surface-2 border border-ct-line rounded-ct-lg p-6 text-center">
                <XCircle className="w-10 h-10 text-ct-mute mx-auto mb-2" />
                <h3 className="font-semibold text-ct-paper">Quote declined</h3>
                <p className="text-sm text-ct-mute-2 mt-1">
              ...
              <div className="bg-ct-surface-2 border border-ct-line rounded-ct-lg p-6 text-center">
                <AlertCircle className="w-10 h-10 text-ct-mute mx-auto mb-2" />
                <h3 className="font-semibold text-ct-paper">This quote is no longer available</h3>
                <p className="text-sm text-ct-mute-2 mt-1">
              ...
              <div className="space-y-2.5">
                <button
                  ...
                  className="w-full py-3.5 bg-ct-teal hover:brightness-110 text-ct-ink font-semibold rounded-ct-md disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                >
                  {accepting && <Loader2 className="w-4 h-4 animate-spin" />}
                ...
                {showDeclineForm ? (
                  <div className="mt-3 rounded-ct-md border border-ct-line bg-ct-surface p-4 space-y-3 text-left">
                    <label className="block text-sm font-medium text-ct-mute-2">
                      Let {businessName} know why <span className="font-normal text-ct-mute">(optional)</span>
                    </label>
                    <textarea {...proseInputProps}
                      ...
                      className="w-full px-3 py-2 border border-ct-line rounded-ct-sm text-sm focus:outline-none focus:ring-2 focus:ring-ct-teal resize-none"
                    />
                    <div className="flex gap-2">
                      <button
                        onClick={() => { setShowDeclineForm(false); setDeclineReason(''); }}
                        className="flex-1 py-2 border border-ct-line text-ct-mute-2 rounded-ct-sm text-sm font-medium hover:bg-ct-surface-2"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        disabled={declining}
                        className="flex-1 py-2 bg-ct-rose hover:brightness-110 text-ct-ink rounded-ct-sm text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                  ...
                  <button
                    ...
                    className="w-full text-center text-sm font-medium text-ct-mute hover:text-ct-mute-2 py-1"
                  >
            ...
            <div className="bg-ct-surface border border-ct-line rounded-ct-lg p-5">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-ct-md bg-ct-teal/[0.14] flex items-center justify-center flex-shrink-0">
