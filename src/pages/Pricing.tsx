    ...
    <div className="min-h-screen bg-ct-surface">
      ...
      <header className="sticky top-0 z-30 bg-ct-surface border-b border-ct-line-soft">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="inline-flex items-center gap-2 text-sm text-ct-mute-2 hover:text-ct-paper transition-colors">
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="text-center mb-4">
          <h1 className="text-3xl sm:text-4xl font-bold text-ct-paper mb-3">
            One fee. Only when you get paid.
          </h1>
          <p className="text-lg text-ct-mute-2 max-w-2xl mx-auto">
          ...
          {tiers.map((t) => {
            ...
            return (
              ...
              >
                ...
                <div className="rounded-ct-md bg-ct-surface-2 border border-ct-line-soft px-4 py-3 mb-5">
                ...
                <ul className="space-y-2.5 mb-7">
                  {(TIER_FEATURES[t.id] ?? []).map((f) => (
                    <li key={f} className="flex items-start gap-2.5">
                      <Check className={`w-4 h-4 flex-shrink-0 mt-0.5 ${popular ? 'text-ct-teal' : 'text-ct-mute'}`} />
                      <span className="text-sm text-ct-mute-2">{f}</span>
                    </li>
                  ))}
                </ul>
                <Link
        ...
        <div className="max-w-3xl mx-auto mb-16">
          <div className="flex items-center gap-2 justify-center mb-2">
            <Calculator className="w-5 h-5 text-ct-teal" />
            <h2 className="text-2xl font-bold text-ct-paper text-center">What would you actually pay?</h2>
          </div>
          <p className="text-sm text-ct-mute text-center mb-6">
            Honest answer — including when Free is your best deal.
          </p>
          <div className="bg-ct-surface-2 rounded-ct-lg border border-ct-line p-6">
            <div className="grid grid-cols-2 gap-4 mb-4 max-w-md mx-auto">
              <div>
                <label className="block text-xs font-medium text-ct-mute mb-1">Average job value ($)</label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium text-ct-mute mb-1">Jobs per month</label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium text-ct-mute mb-1">Materials (% of job)</label>
                <input
            ...
            {calc && calc.materialsCents > 0 && (
              <p className="text-xs text-ct-mute text-center mb-4">
                Of each {dollars(calc.labourCents + calc.materialsCents, 2)} job:{' '}
                <span className="font-medium text-ct-mute-2">{dollars(calc.labourCents, 2)}</span> labour
                {' '}(what we charge on) +{' '}
                <span className="font-medium text-ct-mute-2">{dollars(calc.materialsCents, 2)}</span> materials
                {' '}(we take nothing)
              </p>
            )}
            ...
            {calc ? (
              <>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-xs text-ct-mute uppercase tracking-wide">
                    ...
                    <tbody className="divide-y divide-ct-line">
                      {calc.rows.map(({ tier: t, feePerJob, monthlyFees, total }) => {
                        ...
                        return (
                          <tr key={t.id} className={best ? 'bg-ct-teal/[0.14]' : ''}>
                            ...
                            <td className="py-2.5 pr-4 tabular-nums text-ct-mute-2">{dollars(feePerJob, 2)}</td>
                            <td className="py-2.5 pr-4 tabular-nums text-ct-mute-2">{dollars(monthlyFees, 2)}</td>
                            <td className="py-2.5 pr-4 tabular-nums text-ct-mute-2">{dollars(t.monthly_price_cents, 2)}</td>
                            <td className="py-2.5 tabular-nums font-semibold text-ct-paper">{dollars(total, 2)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <p className="mt-4 text-sm text-ct-mute-2 text-center">
              ...
              <p className="text-sm text-ct-mute text-center">Enter a job value and monthly volume to compare plans.</p>
            )}
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto mb-16">
          <h2 className="text-2xl font-bold text-ct-paper text-center mb-6">How that compares</h2>
          <div className="overflow-x-auto rounded-ct-lg border border-ct-line">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ct-line-soft">
                {COMPETITORS.map((c) => (
                  <tr key={c.name} className={c.highlight ? 'bg-ct-teal/[0.14]' : 'bg-ct-surface'}>
                    ...
                    <td className="px-4 py-3 text-ct-mute-2 whitespace-nowrap">{c.escrow}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="mt-2 text-xs text-ct-mute text-center">
        ...
        <div className="max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4 mb-14">
          <div className="bg-ct-surface-2 rounded-ct-md border border-ct-line p-5 text-center">
            <Percent className="w-6 h-6 text-ct-teal mx-auto mb-2" />
            <h3 className="font-semibold text-ct-paper mb-1 text-sm">One fee, capped</h3>
          ...
          <div className="bg-ct-surface-2 rounded-ct-md border border-ct-line p-5 text-center">
            <Check className="w-6 h-6 text-ct-teal mx-auto mb-2" />
            <h3 className="font-semibold text-ct-paper mb-1 text-sm">Clients pay nothing</h3>
            <p className="text-xs text-ct-mute-2">No client-side platform fees, ever.</p>
          </div>
        </div>
        ...
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-ct-mute">No credit card required. Upgrade or downgrade anytime.</p>
        </div>
      </main>
    </div>
  );
