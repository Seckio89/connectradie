    ...
    <Modal isOpen={isOpen} onClose={onDismiss} maxWidth="md">
      <div className="p-6">
        <div className="flex items-start gap-3">
          {/* Map pin on a house */}
          <div className="relative w-12 h-12 rounded-ct-lg bg-ct-surface-2 flex items-center justify-center flex-shrink-0">
            <Home className="w-6 h-6 text-ct-mute-2" />
            <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-ct-teal flex items-center justify-center ring-2 ring-white">
              <MapPin className="w-3 h-3 text-ct-ink" />
            </span>
          </div>
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-ct-paper">Location access keeps your jobs on track</h2>
            <p className="text-sm text-ct-mute mt-0.5">Optional — you can turn this off any time in Settings.</p>
          </div>
        </div>

        <div className="mt-5">
          <p className="text-sm font-medium text-ct-paper mb-2">ConnecTradie uses your location to:</p>
          <ul className="space-y-2">
            {USES.map((u) => (
              <li key={u} className="flex items-start gap-2.5">
                <CheckCircle2 className="w-4 h-4 text-ct-teal flex-shrink-0 mt-0.5" />
                <span className="text-sm text-ct-mute-2 leading-relaxed">{u}</span>
              </li>
            ))}
          </ul>
        </div>

        <div className="mt-4 rounded-ct-md bg-ct-teal/[0.14] border border-ct-teal/30 p-3">
        ...
        <p className="mt-3 text-sm text-ct-mute-2 leading-relaxed">
          To do this, <span className="font-semibold text-ct-paper">ConnecTradie collects your location in
        ...
        <div className="mt-4 flex items-start gap-2.5 rounded-ct-md bg-ct-amber/[0.13] border border-ct-amber/[0.34] p-3">
          <AlertTriangle className="w-4 h-4 text-ct-amber flex-shrink-0 mt-0.5" />
          <p className="text-sm text-ct-paper leading-relaxed">
        ...
        <div className="mt-6 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
          <button
            type="button"
            onClick={onDismiss}
            className="px-4 py-2.5 min-h-[44px] border border-ct-line text-ct-mute-2 font-medium rounded-ct-md hover:bg-ct-surface-2 transition-colors"
          >
            Not now
          </button>
          <button
            type="button"
            onClick={onAllow}
            className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 min-h-[44px] bg-ct-teal text-ct-ink font-semibold rounded-ct-md hover:brightness-110 transition-colors"
          >
