...
function SiteModal({
  ...
  const input = 'w-full px-3 py-2.5 border border-ct-line rounded-ct-md text-sm focus:outline-none focus:ring-2 focus:ring-ct-teal';
    ...
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="md">
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-ct-paper">{site ? 'Edit location' : 'Add location'}</h2>
        </div>

        <div>
          <label className="block text-sm font-medium text-ct-mute-2 mb-1.5">Location name</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-ct-mute-2 mb-1.5">Contact email <span className="text-ct-mute font-normal">(optional)</span></label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Defaults to main email" className={input} />
          </div>
          <div>
            <label className="block text-sm font-medium text-ct-mute-2 mb-1.5">Contact phone <span className="text-ct-mute font-normal">(optional)</span></label>
        ...
        <div>
          <label className="block text-sm font-medium text-ct-mute-2 mb-1.5">Notes <span className="text-ct-mute font-normal">(optional)</span></label>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button onClick={save} disabled={saving}
            className="flex-1 px-4 py-2.5 bg-ct-teal text-ct-ink rounded-ct-md font-medium hover:brightness-110 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-semibold text-ct-paper">Locations</h2>
          {sites.length > 0 && (
            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-ct-surface-2 text-ct-mute-2">{sites.length}</span>
          )}
        </div>
        <button
          onClick={() => { setEditing(null); setModalOpen(true); }}
          className="inline-flex items-center gap-1.5 px-3 py-2 border border-ct-line rounded-ct-sm text-sm font-medium text-ct-mute-2 hover:bg-ct-surface-2 transition-colors"
        >
          <Plus className="w-4 h-4 text-ct-mute-2" /> Add location
        </button>
      </div>

      {loading ? (
        <div className="flex items-center gap-2 text-sm text-ct-mute py-4"><Loader2 className="w-4 h-4 animate-spin" /> Loading locations…</div>
      ) : sites.length === 0 ? (
        <div className="bg-ct-surface border border-ct-line-soft rounded-ct-lg shadow-sm px-6 py-8 text-center">
          <MapPin className="w-8 h-8 text-ct-mute mx-auto mb-2" />
          <p className="text-sm text-ct-mute">No locations saved. Add their home, office or any site you work at.</p>
        </div>
      ) : (
            ...
            <div key={s.id} className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  {s.address && (
                    <p className="flex items-center gap-1.5 text-sm text-ct-mute mt-1">
                      <MapPin className="w-3.5 h-3.5 text-ct-mute flex-shrink-0" /> <span className="truncate">{s.address}</span>
                    </p>
                  )}
                  <div className="flex items-center gap-3 flex-wrap mt-0.5">
                    {s.contact_email && (
                      <span className="flex items-center gap-1 text-xs text-ct-mute"><Mail className="w-3 h-3 text-ct-mute" /> {s.contact_email}</span>
                    )}
                    {s.contact_phone && (
                      <span className="flex items-center gap-1 text-xs text-ct-mute"><Phone className="w-3 h-3 text-ct-mute" /> {s.contact_phone}</span>
                    )}
                  </div>
                  {s.notes && <p className="text-xs text-ct-mute mt-1">{s.notes}</p>}
                </div>
