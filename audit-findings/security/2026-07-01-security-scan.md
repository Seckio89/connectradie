...
- **FIXED — `LicenseCertificate.tsx` print-window `<title>` injection.** `src/components/LicenseCertificate.tsx:59` interpolated the free-text `holderName` (`profiles.full_name`, from `VerificationCenter.tsx`) raw into the print-window `<title>` written via `document.write` — an RCDATA breakout (`</title>…`) would execute markup in the same-origin print window. Now HTML-escaped via the shared `src/lib/escapeHtml.ts` helper. (The body at `:309` writes `printContent.innerHTML`, which is already-escaped React DOM and is correctly left un-escaped.) Self-XSS only today (holder prints their own certificate), but the same class as the `Payouts.tsx`/`InvoiceViewModal.tsx` fixes. **Correction:** the prior note here wrongly cleared this file as having "no user-controlled interpolation" — it reviewed only the `:309` body and missed the raw `<title>` at `:59`.
- **FIXED — additional print/export `document.write` sinks hardened.** The same escaping was applied to every other user/DB-controlled interpolation in the invoice/receipt/statement print builders: `src/pages/Leads.tsx` (`handleExportSingleInvoice` — incl. cross-user `tradie_name` from payment metadata — and `handleExportInvoice`) and `src/pages/PaymentHistory.tsx` (`handleExportReceiptPDF` — incl. homeowner-authored `jobs.description` visible to the tradie). The duplicated `escapeHtml` helpers in `Payouts.tsx` and `InvoiceViewModal.tsx` were consolidated into `src/lib/escapeHtml.ts`.
