...
| 19 | LOW | Nav | react-helmet inert: 19 pages render `<SEO>` titles that never apply; `PAGE_TITLES` in App.tsx is the real source (baselined N7). Fix helmet or remove the dead `<SEO>` calls | OPEN |
